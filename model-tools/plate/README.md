...
HyperLPR3 v20230229（Apache-2.0，可商用）车牌识别模型，**已直接入库**，无需下载脚本。

## 模型清单（models/）

| 文件 | 大小 | 说明 |
|------|------|------|
| `y5fu_320x_sim.onnx` | ≈2.2 MB | 车牌检测（YOLOv5 多任务，320 输入） |
| `y5fu_640x_sim.onnx` | ≈3.7 MB | 车牌检测（640 输入，大图可选） |
| `rpv3_mdict_160_r3.onnx` | ≈9.8 MB | CRNN + SVTR 车牌字符识别（77 token CTC） |
| `litemodel_cls_96x_r1.onnx` | ≈1.5 MB | 车牌颜色分类（3 类：黄 / 蓝 / 绿） |
