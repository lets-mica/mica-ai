...
import net.dreamlu.mica.ai.tts.engine.*;
...
public class KokoroTts implements AutoCloseable {
	...
	public TtsResult synthesize(String text, String voice, float speed) throws Exception {
		if (text == null || text.trim().isEmpty()) {
			return new TtsResult(new float[0], KokoroTtsConfig.SAMPLE_RATE, 0);
		}
		...
		double duration = (double) audio.length / KokoroTtsConfig.SAMPLE_RATE;

		return new TtsResult(audio, KokoroTtsConfig.SAMPLE_RATE, duration);
	}
